...
{
	char res;
	if (x < 0 || x >= width) return SEAT;
	if (y < 0 || y >= lcount) return SEAT;
	res = table[x][y];
		...
		for (k = 0; k < lcount; k++)
			for (i = 0; i < width; i++)
				if (table[i][k] == SEAT)
				{
					...
					if (getcell(table, i+1, k-1, lcount, width) != SEAT) continue;
					if (getcell(table, i-1, k, lcount, width) != SEAT) continue;
					if (getcell(table, i+1, k, lcount, width) != SEAT) continue;
					if (getcell(table, i-1, k+1, lcount, width) != SEAT) continue;
					if (getcell(table, i, k+1, lcount, width) != SEAT) continue;
					if (getcell(table, i+1, k+1, lcount, width) != SEAT) continue;
					...
					if (getcell(table, i+1, k-1, lcount, width) == OCCP) thrslhd++;
					if (getcell(table, i-1, k, lcount, width) == OCCP) thrslhd++;
					if (getcell(table, i+1, k, lcount, width) == OCCP) thrslhd++;
					if (getcell(table, i-1, k+1, lcount, width) == OCCP) thrslhd++;
					if (getcell(table, i, k+1, lcount, width) == OCCP) thrslhd++;
					if (getcell(table, i+1, k+1, lcount, width) == OCCP) thrslhd++;
